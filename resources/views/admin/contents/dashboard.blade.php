            ...
            <div class="flex items-center justify-between h-16">
                <nav class="flex" aria-label="Breadcrumb">
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        <li class="inline-flex items-center">
                            <a href="{{ route('admin.dashboard') }}" class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 transition-colors duration-200">
                ...
                <div class="flex items-center space-x-4">
                    <div class="text-sm text-gray-600">
                        ...
                        <span class="text-gray-500">{{ now()->setTimezone('Asia/Jakarta')->format('d M Y, H:i') }}</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white shadow-lg border-b-4 border-gray-300">
            <div class="overflow-hidden whitespace-nowrap relative">
                <div class="animate-marquee inline-block py-3 text-base font-medium">
                    ...
                    @foreach($pendingBorrowingRequests as $request)
                        <span class="mr-12 bg-gray-100 px-3 py-1 rounded-2xl border border-gray-300">
                            📋 <strong class="text-gray-900">{{ $request->user->name }}</strong> mengajukan peminjaman 
                            <strong class="text-blue-600">{{ $request->item->nama }}</strong> 
                            ({{ $request->jumlah }} unit) - 
                            <em class="text-gray-600">{{ $request->created_at->setTimezone('Asia/Jakarta')->diffForHumans() }}</em>
                        </span>
                    @endforeach
                    @if($pendingBorrowingRequests->count() > 1)
                        <span class="mr-12 bg-red-100 text-red-800 px-4 py-2 rounded-2xl font-bold animate-pulse border-2 border-red-300">
                            ⚠️ TOTAL {{ $pendingBorrowingRequests->count() }} PERMINTAAN MENUNGGU PERSETUJUAN!
                        </span>
                    @endif
                    <span class="mr-12 bg-white text-red-600 px-4 py-2 rounded-2xl font-bold hover:bg-yellow-200 transition-colors border-2 border-gray-200">
                        👉 <a href="{{ route('admin.borrowing-requests.pending') }}" class="no-underline">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="bg-white/60 backdrop-blur-sm rounded-2xl border border-white/20 shadow-xl p-8 mb-8">
            <div class="flex items-center justify-between">
                    ...
                    <div>
                        ...
                        <div class="flex items-center mt-3 space-x-4">
                            ...
                            <div class="flex items-center text-sm text-gray-500">
                                ...
                                <span>{{ now()->setTimezone('Asia/Jakarta')->format('H:i:s') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="text-right">
                    ...
                    <div class="text-3xl font-bold text-gray-900">{{ number_format($jumlahJenisBarang + $userCount + $supplierCount + $categoryCount) }}</div>
        ...
        <div class="mb-8">
            <div class="bg-white/60 backdrop-blur-sm rounded-2xl border border-white/20 shadow-xl p-6">
                ...
                @include('admin.components.libraries.statisticCard')
            </div>
        </div>
            ...
            <div class="lg:col-span-2 bg-white/60 backdrop-blur-sm rounded-2xl border border-white/20 shadow-xl p-6">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <div class="flex space-x-2">
                        <button onclick="updateChart('weekly')" class="px-3 py-1 text-sm bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-colors duration-200 {{ $range === 'weekly' ? 'bg-blue-500 text-white' : '' }}">
                            Weekly
                        </button>
                        <button onclick="updateChart('monthly')" class="px-3 py-1 text-sm bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-colors duration-200 {{ $range === 'monthly' ? 'bg-blue-500 text-white' : '' }}">
                            Monthly
                        </button>
                        <button onclick="updateChart('yearly')" class="px-3 py-1 text-sm bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-colors duration-200 {{ $range === 'yearly' ? 'bg-blue-500 text-white' : '' }}">
                            Yearly
                        </button>
                    </div>
                </div>
            ...
            <div class="bg-white/60 backdrop-blur-sm rounded-2xl border border-white/20 shadow-xl p-6">
                ...
                <div class="mt-4 space-y-2">
                    <div class="flex items-center justify-between text-sm">
                        ...
                        <span class="font-medium text-gray-900">{{ $jumlahJenisBarang }}</span>
                    </div>
                    <div class="flex items-center justify-between text-sm">
                        <div class="flex items-center">
                        ...
                        <span class="font-medium text-gray-900">{{ $userCount }}</span>
                    </div>
                    <div class="flex items-center justify-between text-sm">
                        <div class="flex items-center">
                        ...
                        <span class="font-medium text-gray-900">{{ $supplierCount }}</span>
                    </div>
                    <div class="flex items-center justify-between text-sm">
                        <div class="flex items-center">
                        ...
                        <span class="font-medium text-gray-900">{{ $categoryCount }}</span>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <tbody class="bg-white/50 divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50/50 transition-colors duration-200">
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $transaksi->created_at->setTimezone('Asia/Jakarta')->format('d M Y H:i') }}
                            </td>
...
<script>
    ...
    const pieChartOptions = {
                    ...
                    labels: {
                        ...
                        total: {
                            ...
                            formatter: function (w) {
                                return {{ $jumlahJenisBarang + $userCount + $supplierCount + $categoryCount }}
                            }
                        },
                        value: {
                            show: true,
        ...
        tooltip: {
            ...
            y: {
                formatter: function(val, opts) {
                    const total = {{ $jumlahJenisBarang + $userCount + $supplierCount + $categoryCount }};
