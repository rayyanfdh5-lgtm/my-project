...
@section('content')
    <div class="form mx-auto space-y-4 p-4 max-w-4xl">
        ...
        <div class="rounded-lg border border-gray-200 bg-white shadow-sm">
            <form action="{{ route('admin.content.savedatausers') }}" method="POST" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                            @foreach ($roles as $value => $role)
                                <div class="flex items-start space-x-3 rounded-lg border {{ $value === 'admin' ? 'border-red-200 bg-red-50' : 'border-blue-200 bg-blue-50' }} p-4 hover:{{ $value === 'admin' ? 'bg-red-100' : 'bg-blue-100' }} transition-colors">
                                    <input id="role_{{ $value }}" name="role" type="radio"
                                        value="{{ $value }}"
                                        {{ old('role') === $value ? 'checked' : '' }}
                                        class="mt-1 h-4 w-4 border-gray-300 {{ $value === 'admin' ? 'text-red-600 focus:ring-red-500' : 'text-blue-600 focus:ring-blue-500' }}">
                                    <div class="flex-1">
                                        <div class="flex items-center space-x-2">
                                            ...
                                            @endif
                                            <label for="role_{{ $value }}" class="block cursor-pointer text-sm font-semibold {{ $value === 'admin' ? 'text-red-900' : 'text-blue-900' }}">
                                                {{ $role['label'] }}
                                            </label>
                                        </div>
                                        <p class="mt-1 text-xs {{ $value === 'admin' ? 'text-red-700' : 'text-blue-700' }}">{{ $role['desc'] }}</p>
                                        @if($value === 'admin')
